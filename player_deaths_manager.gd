...
func _on_deaths_updated(room: String, death_list: Array[String]) -> void:
	...
	for name in death_list:
		if name not in requested_deaths:
			requested_deaths[name] = false
	...
	while len(queued_requests) < MAX_DEATHS_LOADED:
		var players := per_player_deaths.keys()
		if len(players) == 0:
			break
		...
		var player_deaths = per_player_deaths[player_key].keys()
...
func _on_death_loaded(room: String, death_key: String, pos: Vector2, footprints: Array[Vector2]) -> void:
...
func _start_next_request() -> void:
	...
	for death_key in queued_requests.keys():
